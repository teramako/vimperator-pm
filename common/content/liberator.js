...
const Liberator = Module("liberator", {
    ...
    commands() {
        ...
        commands.add(["smi[le]"], "holy-grail",
            function (args) {
                const SMILE = [
                    '                            oooo$$$$$$$$$$$$oooo',
                    '                        oo$$$$$$$$$$$$$$$$$$$$$$$$o',
                ...
                let str = xml`<pre style="background:black;color:white;">${
                    template.map2(xml, SMILE, line => template.maybeXML(line.replace(/[^\s"]+/g,
                        str => `<span style="background:yellow; color:black;">${str}</span>`)) , xml`&#xa;`)
                }</pre>`;
